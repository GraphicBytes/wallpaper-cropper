...
if ($logged_in_id == 0) {
  ...
  if ($logged_in_id == $owner) {
    ...
    if ($checkwidth > $targ_w and $checkheight >$targ_h){
        ...
        $db->sql( "UPDATE wall_generation SET 5by4_share=? WHERE id=?", 'si' , $Ifileroot, $iid );
    ...
    if ($checkwidth > $targ_w and $checkheight >$targ_h){
        ...
        $db->sql( "UPDATE wall_generation SET 5by4thumb=? WHERE id=?", 'si' , $Ifileroot, $iid );
    ...
    if ($checkwidth > $targ_w and $checkheight >$targ_h){
        ...
        $db->sql( "UPDATE wall_generation SET 1280x1024=? WHERE id=?", 'si' , $Ifileroot, $iid );
    ...
    if ($checkwidth > $targ_w and $checkheight >$targ_h){
        ...
        $db->sql( "UPDATE wall_generation SET 2560x2048=? WHERE id=?", 'si' , $Ifileroot, $iid );
    ...
    $current_time = time();
    $db->sql( "UPDATE wall_generation SET 5by4_recrop='1' WHERE id=?", 'i' , $iid );

    $done_5by4_recrop = 1;

    $location = "Location: " . $base_url . "/mobileselect/" . $iid . "/";
