...
if ($logged_in_id == 0) {
  ...
  if ($logged_in_id == $owner) {
    ...
    if ($checkwidth > $targ_w and $checkheight >$targ_h){
        ...
        $db->sql( "UPDATE wall_generation SET 16by9_share=? WHERE id=?", 'si' , $Ifileroot, $iid );
    ...
    if ($checkwidth > $targ_w and $checkheight >$targ_h){
        ...
        $db->sql( "UPDATE wall_generation SET thumbnail=? WHERE id=?", 'si' , $Ifileroot, $iid );
    ...
    if ($checkwidth > $targ_w and $checkheight >$targ_h){
        ...
        $db->sql( "UPDATE wall_generation SET 1280x720=? WHERE id=?", 'si' , $Ifileroot, $iid );
    ...
    if ($checkwidth > $targ_w and $checkheight >$targ_h){
        ...
        $db->sql( "UPDATE wall_generation SET 1920x1080=? WHERE id=?", 'si' , $Ifileroot, $iid );
    ...
    if ($checkwidth > $targ_w and $checkheight >$targ_h){
        ...
        $db->sql( "UPDATE wall_generation SET 2560x1440=? WHERE id=?", 'si' , $Ifileroot, $iid );
    ...
    if ($checkwidth > $targ_w and $checkheight >$targ_h){
        ...
        $db->sql( "UPDATE wall_generation SET 3840x2160=? WHERE id=?", 'si' , $Ifileroot, $iid );
    ...
    $current_time = time();
    $db->sql( "UPDATE wall_generation SET 16by9_recrop='1' WHERE id=?", 'i' , $iid );

    $done_16by9_recrop = 1;

    $location = "Location: " . $base_url . "/16by10select/" . $iid . "/";
